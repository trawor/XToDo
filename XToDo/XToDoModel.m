...
+(XToDoItem*)itemFromLine:(NSString*)line{
  ...
  // Everything on the line after the keyword becomes description content shown in the ToDo List window.
  // Glue the string back together in case there were delimeters after the type string
  // Gratiutous colons and leading / trailing white space is already trimmed by the regex in find.sh
  NSString *trailingComment = @"";
  for (NSUInteger i = 3; i < lineComponents.count; i++) {
    trailingComment = [trailingComment stringByAppendingString:lineComponents[i]];
  }
  ...
  if ([trailingComment isEqualToString:@""]) {
    trailingComment = [NSString stringWithFormat:@"Line %lu", (unsigned long)item.lineNumber];
  }
